...
class VibeAgentSession(BaseModel):
    ...
    user_id = Column(UUIDType(as_uuid=True), nullable=True)  # 可选，用户标识
    ...
    conversation_history = Column(JSONType, default=list)  # 对话历史
    collected_info = Column(JSONType, default=dict)  # 收集的信息
    current_state = Column(String(20), default="gathering")  # gathering/generating/confirming/completed
    llm_config = Column(JSONType, default=dict)  # LLM配置

    # 关联的工作流（与 VibeAgentWorkflow.session_id 构成互引，use_alter 打破建表排序环）
    workflow_id = Column(UUIDType(as_uuid=True), ForeignKey("vibe_agent_workflows.id", ondelete="SET NULL", use_alter=True), nullable=True, index=True)
...
class VibeAgentWorkflow(BaseModel):
    ...
    session_id = Column(UUIDType(as_uuid=True), ForeignKey("vibe_agent_sessions.id", ondelete="SET NULL"), nullable=True, index=True)
    name = Column(String(200), nullable=False)  # 工作流名称
    description = Column(Text, nullable=True)  # 工作流描述

    # LangGraph 图定义
    graph_definition = Column(JSONType, default=dict)  # 完整的图定义（包含state定义等）
    nodes = Column(JSONType, default=list)  # 节点列表 [{id, type, name, config}]
    edges = Column(JSONType, default=list)  # 边列表 [{from, to, condition}]
    ...
    # LLM配置（独立配置）
    llm_config = Column(JSONType, default=dict)  # {api_url, api_key, model, temperature, max_tokens}
...
class VibeAgentWorkflowVersion(BaseModel):
    ...
    workflow_id = Column(UUIDType(as_uuid=True), ForeignKey("vibe_agent_workflows.id", ondelete="CASCADE"), nullable=False, index=True)
    version = Column(Integer, nullable=False)

    # 版本内容
    graph_definition = Column(JSONType, default=dict)
...
class VibeAgentExecution(BaseModel):
    ...
    workflow_id = Column(UUIDType(as_uuid=True), ForeignKey("vibe_agent_workflows.id", ondelete="CASCADE"), nullable=False, index=True)
    workflow_version_id = Column(UUIDType(as_uuid=True), ForeignKey("vibe_agent_workflow_versions.id", ondelete="SET NULL"), nullable=True)

    # 输入输出
    input_data = Column(JSONType, default=dict)  # 输入参数
    output_data = Column(JSONType, default=dict)  # 输出结果
    intermediate_results = Column(JSONType, default=dict)  # 中间结果（各节点执行结果）
    ...
    node_execution_times = Column(JSONType, default=dict)  # 各节点执行时间
...
class VibeAgentNodeConfig(BaseModel):
    ...
    # 节点配置
    config = Column(JSONType, default=dict)  # 节点具体配置
    prompt_template = Column(Text, nullable=True)  # Prompt模板（LLM节点）
    parameters = Column(JSONType, default=dict)  # 参数配置（可调优）

    # 节点输入输出定义
    input_schema = Column(JSONType, default=dict)  # 输入数据结构
    output_schema = Column(JSONType, default=dict)  # 输出数据结构
...
class VibeAgentSessionMessage(BaseModel):
    ...
    session_id = Column(UUIDType(as_uuid=True), ForeignKey("vibe_agent_sessions.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    # 关联数据
    related_workflow_id = Column(UUIDType(as_uuid=True), nullable=True)
    related_node_id = Column(String(100), nullable=True)

    # 元数据
    message_metadata = Column(JSONType, default=dict)
